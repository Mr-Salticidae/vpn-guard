...
**For them**: zip the folder, send it over, and tell them to double-click the file for their OS:

```
一键体检.cmd          <- Windows users double-click this
一键体检.command      <- macOS users double-click this
```

It runs the audit, asks 9 questions about their account, and writes a report to their **Desktop**.
Linux has no double-click convention — just `bash ./field-report.sh`.

> **On macOS the first double-click will be blocked, and that is normal** — send this line along
> with the zip: in Finder, **right-click `一键体检.command` → Open**, then click "Open" again in
> the dialog. Once only. (macOS refuses downloaded scripts by default with "unidentified developer".)
> For Windows users: before extracting, **right-click the zip → Properties → tick "Unblock"**,
...
> are generated by many people and compared by one, so the generator must be cross-platform while
> the comparer just follows whoever is collecting.
>
> ⚠️ **If a Unix report says `出口基准可信: 否`** (system proxy whose address the script cannot read
> — PAC, or auth-required), then that report's exit country / ASN / ISP / timezone delta describe
> their **direct** connection, not their proxy, and must not be compared. The tool calls out which
> reports those are in red and tells you to have them re-run with TUN on.
