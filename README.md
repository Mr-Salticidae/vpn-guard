...
语言一致性、DNS 解析路径是否漏到本地 ISP。换节点或换国家后重跑一次即可。
...
| DNS | 独立 Chrome 配置里关闭"安全 DNS(DoH)"，强制走系统 DNS（TUN 模式=fake-ip 隧道；系统代理模式下域名由代理远端解析），避免浏览器自行解析泄露 | 同左 |
| IP / WebRTC | 由 TUN / 系统代理 / `--proxy` 接管，脚本负责审查接管方式并提示 WebRTC / IPv6 泄露面 | 同左 |
